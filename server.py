...
def get_products():
    return supabase_request("products?select=*&order=created_at.desc") or []

def create_product(product):
    result = supabase_request("products", "POST", product, "return=representation")
...
def get_admin():
    result = supabase_request("app_config?key=eq.admin&select=value") or []
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def is_admin(self):
        clean_sessions()
        token = parse_cookies(self.headers.get("Cookie")).get("ae_session")
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/api/admin/products":
                ...
                return self.send_json(get_products())

            if path == "/api/health":
                supabase_request("products?select=id&limit=1")
                return self.send_json({"ok": True, "database": "Supabase", "connected": True})

        except Exception as error:
            print("ERROR DATABASE:", error)
            return self.send_json({"error": "No se pudo acceder a la base de datos"}, 500)
        ...
        try:
            if path == "/api/admin/setup":
                if get_admin() is not None:
                    return self.send_json({"error": "El administrador ya fue configurado"}, 409)

                password = str(body.get("password", ""))
                if len(password) < 8:
                    return self.send_json({"error": "La contraseña debe tener al menos 8 caracteres"}, 400)

                salt, digest = hash_password(password)
                save_admin({"username": "admin", "salt": salt, "digest": digest})
                return self.send_json({"ok": True})

            if path == "/api/admin/login":
                admin = get_admin()
                if not admin:
                    return self.send_json({"error": "Primero configurá el administrador"}, 400)

                username = body.get("username")
                password = str(body.get("password", ""))
                correct = (
                    username == admin.get("username")
                    and verify_password(password, admin.get("salt", ""), admin.get("digest", ""))
                )
                if not correct:
                ...
                cookie = (
                    f"ae_session={token}; HttpOnly; SameSite=Strict; "
                    f"Secure; Path=/; Max-Age={SESSION_TTL}"
                )
            ...
            if path == "/api/admin/logout":
                ...
                return self.send_json(
                    {"ok": True},
                    cookie="ae_session=; HttpOnly; SameSite=Strict; Secure; Path=/; Max-Age=0",
                )
            ...
            if path == "/api/admin/products":
                ...
                if not name or not brand:
                    return self.send_json({"error": "Nombre y marca son obligatorios"}, 400)
                ...
                product = {
                    ...
                    "gender": str(body.get("gender", "Unisex")).strip(),
                    "description": str(body.get("description", "")).strip(),
                    "notes": str(body.get("notes", "")).strip(),
                    "image": body.get("imageData", ""),
                    "featured": bool(body.get("featured", False)),
                }
                return self.send_json(create_product(product), 201)

        except Exception as error:
            print("ERROR DATABASE:", error)
            return self.send_json({"error": "Error de base de datos"}, 500)
    ...
    def do_PUT(self):
        ...
        try:
            ...
            for key in ["name", "brand", "size", "gender", "description", "notes"]:
                if key in body:
                    updates[key] = str(body.get(key, "")).strip()
            ...
            if "featured" in body:
                updates["featured"] = bool(body["featured"])
            if body.get("imageData"):
                updates["image"] = body["imageData"]
...
if __name__ == "__main__":
    ...
    print(f"Aurora Essence disponible en puerto {port}")
    print("Base de datos: Supabase persistente")
